...
def create_attendance_session(db: Session, course_id: int, title: str) -> None:
    course = db.query(models.Course).filter(models.Course.id == course_id).first()
    students = course.students
    timestamp = datetime.datetime.now()
    attendance_records = [
        models.Attendance(
            course_id=course_id, student_id=student.id, timestamp=timestamp, title=title
        )
...
def mark_attendance(db: Session, course_id: int, student_id: int, class_date: str):
    ...
    attendance = (
        ...
        .filter(models.Attendance.timestamp == class_date_obj)
        .first()
    )
    print(attendance.id, attendance.course_id, attendance.student_id, attendance.present)
    if attendance:
